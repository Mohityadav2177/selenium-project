...
PROCEDURE: \n
1. Enable the Telnet server by configuring the  aaa authentication login telnet local command in configuration mode.
3. Access the device using  telnet  <ip_address>.
4. Configure a new user account on the device with the command username hfcl privilege 15 password unencrypted Discover@1234.
5.Establish an telnet session to the device using the credentials of user hfcl for Privileged EXEC Mode.
6. Validate successful user login by executing the show users command to confirm the session is active.
7. Disable telnet server by configuring the no aaa authentication login telnet local command in configuration mode.
